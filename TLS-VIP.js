...
const targetPort = parsedTarget.protocol === 'https:' ? 443 : 80;
    ...
    console.clear();
console.log('TLS-HTTPS : @duongngocphong0311'.rainbow);
console.log(`Target : ${parsedTarget.host} | Port : ${targetPort}`.rainbow);
console.log(`Time : ${args.time.toString()}/300 Second`.rainbow);
console.log(`Threads : ${args.threads.toString()}/256`.rainbow);
console.log(`Rate : ${args.Rate.toString()}/1024`.rainbow);
console.log(`Proxy : ${args.proxyFile} | ${proxies.length}`.rainbow);
console.log(`User : ngocphong0311 | Expiry : 99/99/9999`.rainbow);
console.log('Support : HTTP1.1/HTTP2/HTTP3/TLSv1.1/TLSv1.2/TLSv1.3'.rainbow);
  setTimeout(() => {
    process.exit(1);
  }, process.argv[3] * 1000);
