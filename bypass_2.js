const net = require("net");
 ...
 if (process.argv.length < 7){console.log(`Usage: node bypass.js target time rate thread proxyfile`.rainbow); process.exit();}
 const headers = {};
...
let randomHeaders = headerGenerator.getHeaders();
 ...
 if (cluster.isMaster){
    ...
    console.log('\x1b[38;2;255;0;0m' +  'Share by Villain'.bgRed);
    console.log(`╔═════════╩═════════════════════════════════╩═════════╗`.gray)
 ...
 headers["cache-control"] = "no-cache", "max-age=0", "only-if-cached","no-transform","must-revalidate","no-store","public","private";
